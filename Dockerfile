...
# Based on fleet-v4.79.1 with modified LoadLicense()
...
FROM golang:1.25.5-bookworm AS backend
...
RUN CGO_ENABLED=1 go build -tags full,fts5,netgo -trimpath \
    ...
    -X github.com/fleetdm/fleet/v4/server/version.version=4.79.1-kencove \
    -X github.com/fleetdm/fleet/v4/server/version.branch=premium/v4.79.1" \
    -o fleet ./cmd/fleet
